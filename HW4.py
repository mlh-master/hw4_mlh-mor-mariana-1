...
#--------------------------Impelment your code here:-------------------------------------
model_relu.compile(optimizer=AdamOpt, metrics=['accuracy'], loss='categorical_crossentropy') #loss = sparse_categorical_crossentropy / kullback_leibler_divergence
history = model_relu.fit(BaseX_train, BaseY_train, batch_size=batch_size, epochs=epochs) #training the model on the training set
stat_metric = model_relu.evaluate(X_test, Y_test, verbose=2)
#print(model_relu.metrics_names)
print('Model loss is ', str(round(stat_metric[0],3)), ' and model accuracy is ', str(round(stat_metric[1],3)))
#----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
new_a_model.compile(optimizer=AdamOpt, metrics=['accuracy'], loss='categorical_crossentropy') #loss = sparse_categorical_crossentropy / kullback_leibler_divergence
history_a = new_a_model.fit(BaseX_train, BaseY_train, batch_size=batch_size, epochs=epochs) #training the model on the training set

if not("results" in os.listdir()): #save the results
    os.mkdir("results")
save_dir = "results/"
model_name = "final_weights.25e"
model_path = os.path.join(save_dir, model_name)
new_a_model.save(model_path)
print('Saved trained model at %s ' % model_path)

stat_metric_a = new_a_model.evaluate(X_test, Y_test, verbose=2)
print('Model loss is ', str(round(stat_metric_a[0],3)),' and model accuracy is ', str(round(stat_metric_a[1],3)))
#-----------------------------------------------------------------------------------------
...
#--------------------------Impelment your code here:-------------------------------------
def get_net(input_shape,drop,dropRate,reg,num_of_filt):
    #Defining the network architecture:
    model = Sequential()
    model.add(Permute((1,2,3),input_shape = input_shape))
    model.add(Conv2D(filters=num_of_filt[0], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_1',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=num_of_filt[1], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_2',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=num_of_filt[2], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_3',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(filters=num_of_filt[3], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_4',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(filters=num_of_filt[4], kernel_size=(3,3), padding='same', activation='relu',name='Conv2D_5',kernel_regularizer=regularizers.l2(reg)))
    if drop:
        model.add(Dropout(rate=dropRate))
    model.add(BatchNormalization(axis=1))
    model.add(MaxPooling2D(pool_size=(2, 2)))
...
num_of_filt = np.array([32, 64, 64, 128, 128])

NNet_new=get_net(input_shape,drop,dropRate,reg,num_of_filt)
